import type { Config } from "tailwindcss";

export default {
  darkMode: ["class"],
  content: ["./pages/**/*.{ts,tsx}", "./components/**/*.{ts,tsx}", "./app/**/*.{ts,tsx}", "./src/**/*.{ts,tsx}"],
  prefix: "",
  theme: {
    container: {
      center: true,
      padding: "2rem",
      screens: {
        "2xl": "1400px",
      },
    },
    extend: {
      fontFamily: {
        display: ['Instrument Serif', 'Noto Serif SC', 'Georgia', 'serif'],
        'display-zh': ['Noto Serif SC', 'STSong', 'SimSun', 'serif'],
        sans: ['Inter', 'Noto Sans SC', 'system-ui', 'sans-serif'],
        'sans-zh': ['Noto Sans SC', 'PingFang SC', 'Microsoft YaHei', 'sans-serif'],
        mono: ['JetBrains Mono', 'SF Mono', 'monospace'],
      },
      fontSize: {
        'hero': ['clamp(2.5rem, 7vw, 5rem)', { lineHeight: '1.05', letterSpacing: '-0.02em' }],
        'headline': ['clamp(1.75rem, 4vw, 2.5rem)', { lineHeight: '1.15', letterSpacing: '-0.01em' }],
        'subhead': ['clamp(1.125rem, 2vw, 1.375rem)', { lineHeight: '1.4' }],
      },
      colors: {
        border: "hsl(var(--border))",
        input: "hsl(var(--input))",
        ring: "hsl(var(--ring))",
        background: "hsl(var(--background))",
        foreground: "hsl(var(--foreground))",
        primary: {
          DEFAULT: "hsl(var(--primary))",
          foreground: "hsl(var(--primary-foreground))",
        },
        secondary: {
          DEFAULT: "hsl(var(--secondary))",
          foreground: "hsl(var(--secondary-foreground))",
        },
        destructive: {
          DEFAULT: "hsl(var(--destructive))",
          foreground: "hsl(var(--destructive-foreground))",
        },
        muted: {
          DEFAULT: "hsl(var(--muted))",
          foreground: "hsl(var(--muted-foreground))",
        },
        accent: {
          DEFAULT: "hsl(var(--accent))",
          foreground: "hsl(var(--accent-foreground))",
        },
        popover: {
          DEFAULT: "hsl(var(--popover))",
          foreground: "hsl(var(--popover-foreground))",
        },
        card: {
          DEFAULT: "hsl(var(--card))",
          foreground: "hsl(var(--card-foreground))",
        },
        abyss: {
          base: "hsl(var(--abyss-base))",
          surface: "hsl(var(--abyss-surface))",
        },
        tide: {
          accent: "hsl(var(--tide-accent))",
          glow: "hsl(var(--tide-glow))",
        },
        bio: {
          teal: "hsl(var(--bio-teal))",
          azure: "hsl(var(--bio-azure))",
        },
        status: {
          rising: "hsl(var(--status-rising))",
          calm: "hsl(var(--status-calm))",
        },
        sidebar: {
          DEFAULT: "hsl(var(--sidebar-background))",
          foreground: "hsl(var(--sidebar-foreground))",
          primary: "hsl(var(--sidebar-primary))",
          "primary-foreground": "hsl(var(--sidebar-primary-foreground))",
          accent: "hsl(var(--sidebar-accent))",
          "accent-foreground": "hsl(var(--sidebar-accent-foreground))",
          border: "hsl(var(--sidebar-border))",
          ring: "hsl(var(--sidebar-ring))",
        },
      },
      borderRadius: {
        lg: "var(--radius)",
        md: "calc(var(--radius) - 2px)",
        sm: "calc(var(--radius) - 4px)",
      },
      keyframes: {
        "accordion-down": {
          from: { height: "0" },
          to: { height: "var(--radix-accordion-content-height)" },
        },
        "accordion-up": {
          from: { height: "var(--radix-accordion-content-height)" },
          to: { height: "0" },
        },
        "caustics": {
          "0%, 100%": { 
            transform: "scale(1) rotate(0deg)",
            opacity: "0.04",
          },
          "50%": { 
            transform: "scale(1.02) rotate(0.5deg)",
            opacity: "0.06",
          },
        },
        "tide-drift": {
          "0%": { transform: "translateX(-3%) translateY(0)" },
          "50%": { transform: "translateX(3%) translateY(-1%)" },
          "100%": { transform: "translateX(-3%) translateY(0)" },
        },
        "subtle-float": {
          "0%, 100%": { transform: "translateY(0)" },
          "50%": { transform: "translateY(-3px)" },
        },
        "ticker-scroll": {
          "0%": { transform: "translateX(0)" },
          "100%": { transform: "translateX(-50%)" },
        },
        "waveform": {
          "0%, 100%": { transform: "scaleY(0.3)" },
          "50%": { transform: "scaleY(1)" },
        },
        "dock-appear": {
          "0%": { transform: "translateX(-50%) translateY(100%)", opacity: "0" },
          "100%": { transform: "translateX(-50%) translateY(0)", opacity: "1" },
        },
      },
      animation: {
        "accordion-down": "accordion-down 0.2s ease-out",
        "accordion-up": "accordion-up 0.2s ease-out",
        "caustics": "caustics 15s ease-in-out infinite",
        "tide-drift": "tide-drift 25s ease-in-out infinite",
        "subtle-float": "subtle-float 4s ease-in-out infinite",
        "ticker-scroll": "ticker-scroll 40s linear infinite",
        "waveform": "waveform 1s ease-in-out infinite",
        "dock-appear": "dock-appear 0.4s ease-out",
      },
    },
  },
  plugins: [require("tailwindcss-animate")],
} satisfies Config;
